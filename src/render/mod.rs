...
pub fn write_event<'a, W: Write>(
    ...
    match (state, event) {
        ...
        (NestedState(return_to, Inline(InlineText, _)), End(Link(Autolink, _, _))) => {
            // When closing an autolink in inline text we just return because
            // the link's already written out (link text and link destination
            // are identical for autolinks)
            Ok((*return_to, data))
        }
